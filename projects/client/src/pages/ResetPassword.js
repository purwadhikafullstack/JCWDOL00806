...
const ResetPassword = () => {
    ...
    const onSubmit = async (values) => {
        try {
            let checkEmail = await axios.get(`${process.env.REACT_APP_API_BASE_URL}/users/userdetail?email=${values.email}`)
            ...
            if (checkEmail.data.data[0].provider !== 'website') {
                ...
                await axios.get(`${process.env.REACT_APP_API_BASE_URL}/users/reset-confirm?email=${values.email}&id=${checkEmail.data.data[0].id}`)
                setConfirmed(true)
            }
        } catch (error) {
            console.log(error)
