  ...
  const OrderList = () => {
    ...
    return orderList.map((order, idx) => {
      ...
      if (order.payment_proof) {
        image = `${process.env.REACT_APP_SERVER_URL}/image/${order.payment_proof
          ?.replace(/"/g, "")
          .replace(/\\/g, "/")}`;
      } else {
        image = undefined;
      ...
      const paymentDeadline = new Date(order.payment_deadline).toLocaleString(
        "en-US",
      ...
      const paymentDeadlineTime = paymentDeadline.slice(
        paymentDeadline.indexOf(" ") + 1
      );
      return (
        <UserOrderCard
          key={idx}
